...
class mydataset_sb(Dataset):
    def __init__(self, td_path, inf_path, segy_path, inline_row, xline_row, xline_start, result_path, frequency):
        ...
        for well_name, values in well_dict.items():
            ...
            feature_maps = data_segy[int(time[0]) : int(time[-1])]
            ...
            new_indices = torch.linspace(0, len(depth_segy) - 1, feature_maps.shape[0])
            ...
            for i in range(feature_maps.shape[0]): 
                if i >= self.frequency/2 and i <= feature_maps.shape[0] - self.frequency/2 :
                    for sublist in result_list:
                        if float(sublist[0]) <= depth_segy[i] and depth_segy[i] <= float(sublist[1]):
                            ...
                            features = feature_maps[int(i - self.frequency/2) : int(i + self.frequency/2)]
...
# mydataloder = DataLoader(mydata_sb_1d, batch_size= 1, shuffle = False)   
# print(len(mydataloder))  
     
# save_dir = "D:\桌面\GD\data\SB\dataset_1d"  # 修改为你想要保存的目录路径
