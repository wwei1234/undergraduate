...
class mydataset_hn(Dataset):
    def __init__(self, pathname, cwts_scale, sampling_rate, inf_path, segyfile, inline_row, xline_row, xline_start, x, y, grid_size, frequency):
                ...
                if i >= self.frequency/2 and i <= len(data_segy) - self.frequency/2 :
                    for sublist in well_data:
                        if float(sublist[0]) <= depth_segy[i] and depth_segy[i] <= float(sublist[1]):
                            ...
                            features = torch.tensor(feature_maps[int(-1-self.frequency):-1, int(i - self.frequency/2 ): int(i + self.frequency/2)])
...
# mydata_hn = mydataset_hn('D:/桌面/2024王威本科毕设/HN/井', 128, 500, 'D:/桌面/2024王威本科毕设/HN/井点坐标.txt','D:/桌面/2024王威本科毕设/HN/CNOOC_DF11_3D_OBN_QPSDM_STK_RAW_D_FEB2023.segy', 189, 193, 2870, 148525.5, 2068537.5, 12.5, 64)        
# mydataloder_hn = DataLoader(mydata_hn, batch_size= 1, shuffle = False)     

# print(len(mydataloder_hn))  
 
        
# save_dir = "D:\桌面\GD\data\HN\dataset"  # 修改为你想要保存的目录路径
